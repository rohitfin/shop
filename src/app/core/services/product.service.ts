...
export class ProductService {
  ...
  arrProducts = [
    ...
    },
    {
      ...
      items_left: 3,
      imageURL:
        'https://imagescdn.reebok.in/img/app/product/9/944109-12101167.jpg?auto=format&w=390',
    ...
    },
    {
      ...
      is_in_inventory: true,
      items_left: 3,
      imageURL: 'https://images.unsplash.com/photo-1619646176605-b7417fb53b1e',
      slug: 'sk80-low',
    },
    {
      id: 34,
      ...
      is_in_inventory: true,
      items_left: 3,
      imageURL: 'https://images.unsplash.com/photo-1512990414788-d97cb4a25db3',
      slug: 'michael-feburary-sk8-hi',
    },
  ];

  observable_Product = of(this.arrProducts);

  allProductCount = 0;
  inStockProductCount = 0;
  outOfStockProductCount = 0;

  cartProduct: WritableSignal<any[]> = signal<Product[]>([]);
  cartCount = computed(() => this.cartProduct().length);
  favoritProduct: WritableSignal<any[]> = signal<Product[]>([]);
  favoritCount = computed(() => this.favoritProduct().length);


  subTotal = computed(() =>
    this.cartProduct().reduce(
      (acc, ele, ind, arr) => acc + ele.price * ele.cartQuantity,
      0
    )
  );
  ...
  discountPrice = computed(() => {
    const amount = this.subTotal() > 1000 ? (this.subTotal() * 15) / 100 : 0;
    return Number(Math.round(amount).toFixed(2));
  });

  totalPriceWithoutSalesTax = computed(() => {
    return Number(
      Math.round(
        this.subTotal() + this.deliveryCharge() - this.discountPrice()
      ).toFixed(2)
    );
  });
  ...
  salesTax = computed(() => {
    return Number(
      Math.round((this.totalPriceWithoutSalesTax() * 18.5) / 100).toFixed(2)
    );
  });
  ...
  addToCartProduct(newProduct: Product): void {
    this.cartProduct.update((products) => [...products, newProduct]);
  }

  removeFromCartProduct(selectedProduct: Product) {
    this.cartProduct.update((products) =>
    ...
    this.cartProduct.update((products) =>
      products.map((p) => {
        if (p.id === product.id) {
          ...
          // p.price = p.price * p.cartQuantity;
        }
        return p;
      })
    );
  ...
  removeProductQuantity(product: Product) {
    this.cartProduct.update((products) =>
      products
        .map((p) => {
          if (p.id === product.id) {
            const leftQuantity = p.cartQuantity - 1;
            // const price = leftQuantity ? p.price * leftQuantity : p.price;
            return { ...p, cartQuantity: leftQuantity };
          }
          return p;
        })
